...
opt           = keras.optimizers.Adam() #lr=0.0001 for the task 4
...
def base_net():
    ...
    ### your code here
    ###
    layer_in = Input(shape=(nx,ny,3)) ### create your input layer here
    ### downsample the data 
    layer = AveragePooling2D(pool_size=dfactor, padding='same')(layer_in)
    output_1 = Flatten()(layer)
    output_2 = Dense(nc,activation='relu')(output_1)
    predictions = Dense(4,activation='softmax')(output_2)
    model = Model(inputs=layer_in, outputs=predictions)
...
def Conv_net():
    ...
    ### downsample the data 
    layer = AveragePooling2D(pool_size=dfactor, padding='same')(layer_in)
    output_1=Conv2D(nc,(3,3),padding='same',activation='relu')(layer)
...
def Conv_sin():
    ...
    ### downsample the data 
    layer = AveragePooling2D(pool_size=dfactor, padding='same')(layer_in)
    output_1=Conv2D(nc,(3,3),padding='same',activation='tanh')(layer)
    output_2 = Flatten()(output_1)
    predictions = Dense(4,activation='softmax')(output_2)
    model = Model(inputs=layer_in, outputs=predictions)
...
def Conv_improved():
    ...
    layer_in = Input(shape=(nx,ny,3)) ### create your input layer here
    ### downsample the data 
    layer = MaxPooling2D(pool_size=dfactor, padding='same')(layer_in)
    output_1=Conv2D(nc*2,(3,3), padding='same', activation='relu')(layer)
    output_2=Conv2D(nc,(3,3), padding='same', activation='relu')(output_1)
    output_3 = Flatten()(output_2)
    predictions = Dense(4,activation='softmax')(output_3)
    model = Model(inputs=layer_in, outputs=predictions)
...
models = { \
    "base"           :  base_net,  
   "Conv"           :  Conv_net,
   "Conv_single"    :  Conv_sin,
   "Conv_improved"  :  Conv_improved
